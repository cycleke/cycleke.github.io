...
显然\\( L = c \times k + a - b \\) 或 \\( L = c \times k - a - b \\)，对于每个 L，$$ step =
lcm(n \times k,L) / L = n \times k / gcd(n \times k, L) $$ c 从 1 取到 n。

# E.Lynyrd Skynyrd

设 begin_p[i]为以 i 结尾的环排列起点的最大位置，用倍增求 begin_p[i],用前缀和优化就可
以解决。
